...
// Selfleak by ian its pasted off neutron so idk :)
/*
      [+] Added antiskid check! game will crash when menu is closed and tabs don't work anymore :)
*/
...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		if (showMenu)
		{
			...
			if (ImGui::Begin(xorstr("XYZ Cheats"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize))
			{
				bool ianiscool = true;
				...
				if (ianiscool)
				{
					...
					static std::vector<Points> drawPoints;

					for (int i = 0; i < get_array_size(points); ++i) {
						int count = 0;
						draw_list->AddCircleFilled(points[i].point, 1.5f, IM_COL32(200, 200, 200, 255));
						for (int y = 0; y < get_array_size(points); ++y) {
							if (distance(points[i].point.x, points[i].point.y, points[y].point.x, points[y].point.y) < 70.f) {
								count++;
								drawPoints.push_back({ points[i], points[y] });
							}
						}
						if (count > 2)
							for (Points n : drawPoints) {
								draw_list->AddLine(n.point1.point, n.point2.point, IM_COL32(150, 148, 152, 255), 1.0f);
							}
						drawPoints.clear();
					}
				}
				...
				// if you sell this i will find you and i will shit on ur name ( just a warning :D )
				if (modulartabs == 0)
				{
					ImGui::Checkbox("Aimbot", &Settings.Aimbot);
					ImGui::Text("     ");
					ImGui::SliderFloat("AimFov", &Settings.AimbotFOV, 0.0f, 250.0f);
					ImGui::Text("     ");
				}
				if (modulartabs == 1)
				{
					ImGui::Checkbox("Box", &Settings.ESP.Boxes);
					ImGui::Text("     ");
					ImGui::Checkbox("Bone", &Settings.ESP.Skeletons);
					
					ImGui::Text("     ");
				}
				if (modulartabs == 2)
				{
					ImGui::Checkbox("Enable Polygon Backround", &ianiscool);
				}
